...
const API_URL = 'http://192.168.1.25:8080/api/roles';
// Service để gọi các API liên quan đến Post
export const createRole = async (role) => {
  console.log('Role Data Sent to Backend:', role);
  try {
    const response = await axios.post(API_URL, role).then((res) => {
...
export const getAllRole = async (post) => {
  try {
...
export const updateRole = async (id, post) => {
  try {
    console.log(id, post);
